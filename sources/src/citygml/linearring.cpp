...
namespace citygml {
    ...
    TVec3d LinearRing::computeNormal() const
    {
        ...
        // Convert latitude, longitude to diff of meter from the first point.
        ...
        constexpr double meter_per_degree = pi2 * r / 360.0;

        for(unsigned i=0; i<len; i++){
            double diff_lat_meter = (m_vertices[i].x - m_vertices[0].x) * meter_per_degree;
            double diff_lon_meter = (m_vertices[i].y - m_vertices[0].y) * meter_per_degree;
            vec_xyz.emplace_back(diff_lat_meter, diff_lon_meter, m_vertices[i].z);
        }
