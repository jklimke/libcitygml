...
namespace citygml
{
    ...
    const std::string AppearanceManager::description() {
        std::stringstream ss;
        ss << "AppearanceManager holding " << _appearances.size() << " and " << _appearancesMap.size() << " in map. Appearances :" << std::endl;
        int i = 0;
        for(auto appearance : _appearances){
            ss << "Appearance No." << i << "ID: " << appearance->getId() << " Type " << appearance->getType() << std::endl;
            i++;
        }
        return ss.str();
    }
...
void CityModel::finish( const ParserParams& params )
{
    ...
    for(auto themePair : _appearanceThemes) {
        std::cout << "Theme " << themePair.first << std::endl;
        
        std::cout << themePair.second->description() << std::endl;
    }
